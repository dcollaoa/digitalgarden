...
Si ejecutamos nuevamente con la opción `-r`, la herramienta intentará resolver los registros desconocidos realizando una consulta `A`. Ahora podemos ver que una dirección IP de `172.16.5.240` apareció para LOGISTICS. Si bien este es un ejemplo pequeño, vale la pena ejecutar esta herramienta en entornos más grandes. Podemos descubrir registros "ocultos" que pueden llevar a descubrir hosts interesantes.
